...
static jmethodID midSetBatteryMonomerNumbers;
static jmethodID midSetBatteryModuleHighTemperature;
static jmethodID midSetBatteryModuleLowTemperature;
static jmethodID midSeBatteryMonomerVoltages;
...
void* thread_fun1(void* arg) {
	...
	jintArray ja_Dw9 = (*env)->NewIntArray(env, 16);
	jintArray ja_DEM2 = (*env)->NewIntArray(env, 19);
	jintArray ja_DBCM = (*env)->NewIntArray(env, 46);
	jintArray ja_DIC = (*env)->NewIntArray(env, 17);
	while (1) {
		nbytes = recv_can(mSocket1, &frame_read1);
...
void* thread_fun2(void* arg) {
	...
	while (1) {
		mSocket2 = open_can("can1");
		if (mSocket2 != -1)
			break;
		sleep(1000);
	}
	while (1) {
		nbytes = recv_can(mSocket2, frame_read2);
...
JNIEXPORT void JNICALL Java_com_mark_bus_data_DataHandler_setWorkMode(JNIEnv* env, jobject thiz,jint mode) {
	screen_send2mode_data s2m = {};
	s2m.mode = mode;
...
JNIEXPORT void JNICALL Java_com_mark_bus_data_DataHandler_setButtons(JNIEnv* env, jobject thiz,	int frontFogButton,int rearFogButton,int hyperbaricCabinButton,int rootLightsButton,int dRootLightsButton,int tvPowerButton,int guideBoardButton,int leftTurnningButton,int rightTurnningButton,int noneButton,	int closeFrontButton,int closeMidButton) {

	srceen_send2_can_data ss2cd = {};
	ss2cd.front_fog_light = frontFogButton;
	LOGD("frontFogButton:  %d \n", frontFogButton);ss2cd.rear_fog_light =rearFogButton;
	ss2cd.high_vol_light =hyperbaricCabinButton;
	ss2cd.top_single_light =rootLightsButton;
	ss2cd.top_double_light =dRootLightsButton;
	ss2cd.tv_power =tvPowerButton;
	ss2cd.electric_road_switch = guideBoardButton;
	ss2cd.front_door_on = closeFrontButton;
	LOGD("closeFrontButton1: %d\n",
		closeFrontButton);ss2cd.front_door_off = !closeFrontButton;
	LOGD("closeFrontButton2: %d\n", !closeFrontButton);
ss2cd.mid_door_on = closeMidButton;
ss2cd.mid_door_off = !closeMidButton;
...
midSetBatteryMonomerNumbers = (*env)->GetStaticMethodID(env, cls, "setBatteryMonomerNumber", "(IIIIIIIII)V");
midSetBatteryModuleHighTemperature = (*env)->GetStaticMethodID(env, cls, "setBatteryModuleHighTemperature", "(IIIII)V");
midSetBatteryModuleLowTemperature = (*env)->GetStaticMethodID(env, cls, "setBatteryModuleLowTemperature", "(IIIII)V");
midSeBatteryMonomerVoltages = (*env)->GetStaticMethodID(env, cls, "setBatteryMonomerVoltages", "(IIIII)V");
...
(*env)->SetObjectArrayElement(env, jintarray, 16,
		(jobject)((int) (pcm->high_vol_light)));
...
(*env)->SetObjectArrayElement(env, jintarray, 35,
		(jobject)((int) (pcm->engine_light)));
(*env)->SetObjectArrayElement(env, jintarray, 36,
		(jobject)((int) (pcm->back_light)));
(*env)->SetObjectArrayElement(env, jintarray, 37,
		(jobject)((int) (pcm->break_light)));
(*env)->SetObjectArrayElement(env, jintarray, 38,
		(jobject)((int) (pcm->rear_fog_light)));
(*env)->SetObjectArrayElement(env, jintarray, 39,
		(jobject)((int) (pcm->relay_power)));
(*env)->SetObjectArrayElement(env, jintarray, 40,
		(jobject)((int) (pcm->wc_light)));
(*env)->SetObjectArrayElement(env, jintarray, 41,
		(jobject)((int) (pcm->third_bridge)));
(*env)->SetObjectArrayElement(env, jintarray, 42,
		(jobject)((int) (pcm->engine_water_level)));
(*env)->SetObjectArrayElement(env, jintarray, 43,
		(jobject)((int) (pcm->rear_compartment_door_inductive_switch)));
(*env)->SetObjectArrayElement(env, jintarray, 44,
		(jobject)((int) (pcm->front_door_open)));
(*env)->SetObjectArrayElement(env, jintarray, 45,
		(jobject)((int) (pcm->mid_door_open)));
...
(*env)->SetObjectArrayElement(env, jintarray, 14,
		(jobject)((int) pdw9->score));
(*env)->SetObjectArrayElement(env, jintarray, 15,
		(jobject)((int) pdw9->avg_currency_used));
...
(*env)->SetObjectArrayElement(env, ja, 17, (jobject)((int) pdw1->break_singal));
...
(*env)->SetObjectArrayElement(env, ja, 24, (jobject)((int) pdw1->water_low));

(*env)->SetObjectArrayElement(env, ja, 25, (jobject)((int) pdw1->water_pump));
 ...
 */
handleBatteryMonomerNumbers(JNIEnv *env, char * pbmn, int protocolNumber) {
(*env)->CallStaticVoidMethod(env, cls, midSetBatteryMonomerNumbers,
		protocolNumber, (int) *pbmn, (int) *(pbmn + 1), (int) *(pbmn + 2),
		(int) *(pbmn + 3), (int) *(pbmn + 4), (int) *(pbmn + 5),
		(int) *(pbmn + 6), (int) *(pbmn + 7));

}
handleBatteryModuleHighTemperature(JNIEnv *env,
	batteryModuleHighTemperature * pbmt, int protocolNumber) {
(*env)->CallStaticVoidMethod(env, cls, midSetBatteryModuleHighTemperature,
		protocolNumber, (int) pbmt->t1, (int) pbmt->t2, (int) pbmt->t3,
		pbmt->t4);
}
handleBatteryModuleLowTemperature(JNIEnv *env,
	batteryModuleHighTemperature * pbmt, int protocolNumber) {
(*env)->CallStaticVoidMethod(env, cls, midSetBatteryModuleLowTemperature,
		protocolNumber, (int) pbmt->t1, (int) pbmt->t2, (int) pbmt->t3,
		pbmt->t4);
}
handleBatteryMonomerVoltages(JNIEnv *env, batteryModuleHighTemperature * pbmt,
	int protocolNumber) {
(*env)->CallStaticVoidMethod(env, cls, midSeBatteryMonomerVoltages,
		protocolNumber, (int) pbmt->t1, (int) pbmt->t2, (int) pbmt->t3,
		(int) pbmt->t4);
}
